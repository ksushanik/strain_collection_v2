...
describe('StorageService', () => {
  ...
  const prisma = {
    storageBox: { findMany: jest.fn(), findUnique: jest.fn(), create: jest.fn() },
  ...
  it('throws when strain not found on allocate', async () => {
    prisma.storageCell.findFirst = jest.fn().mockResolvedValue({ id: 1, boxId: 1, cellCode: 'A1' });
  ...
  it('updates primary flag when same strain already allocated', async () => {
    prisma.storageCell.findFirst = jest.fn().mockResolvedValue({ id: 1, boxId: 1, cellCode: 'A1', strain: {} });
    ...
    const res = await service.allocateStrain({ boxId: 1, cellCode: 'A1', strainId: 5, isPrimary: true });
  ...
  it('reassigns when different strain occupies cell', async () => {
    prisma.storageCell.findFirst = jest.fn().mockResolvedValue({ id: 1, boxId: 1, cellCode: 'A1', strain: { id: 2 } });
    prisma.strain.findUnique = jest.fn().mockResolvedValue({ id: 5 });
    prisma.strainStorage.findUnique = jest.fn().mockResolvedValue({ id: 11, strainId: 2 });
    ...
    const res = await service.allocateStrain({ boxId: 1, cellCode: 'A1', strainId: 5, isPrimary: false });

    expect(prisma.strainStorage.delete).toHaveBeenCalledWith({ where: { cellId: 1 } });
    expect(prisma.storageCell.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { status: 'OCCUPIED' } });
  ...
  it('deallocates and marks cell free', async () => {
    ...
    expect(prisma.storageCell.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { status: 'FREE' } });
    expect(prisma.strainStorage.delete).toHaveBeenCalledWith({ where: { id: 99 } });
