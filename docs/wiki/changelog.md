...
## 2025-12-11
- **AdminJS Access Control**: добавлен компонент `PermissionsGrid` для удобной настройки прав (чекбоксы вместо JSON) в редакторе Ролей и Групп. Исправлена логика отображения прав при чтении из базы.
- **Security**: ужесточены права по умолчанию для роли `USER` (только чтение, без создания/редактирования).
