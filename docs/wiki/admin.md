...
- **Role/Group**: права настраиваются через таблицу с чекбоксами (PermissionsGrid).
- **Логика приоритетов**:
  1. Группа (Group Permissions)
  2. Роль (Role Permissions)
  3. Default (вшито в код)
- **PermissionsGrid**: позволяет гибко включать/выключать `read`, `create`, `update`, `delete`, `manage` для каждого ресурса. Пустые чекбоксы означают отсутствие прав на уровне данной сущности (и переход к следующему приоритету).
